...
def proper_length_ratio_reward(
    ...
    Args:
        completions1: List of len g text completions (groups) from the first agent
        completions2: List of len g text completions (groups) from the second agent
        target_min: Agent 2 should be at least target_min times longer than agent 1
        target_max: Agent 2 should be at most target_max times longer than agent 1

    Returns:
        List of len g reward scores based on each pair of completions (groups).
    """
    rewards = []
...
def example_usage():
    ...
    config = MAGRPOConfig(
        output_dir="./magrpo_multi_reward_output",  # Model outputs (checkpoints)
        num_train_epochs=3,
        per_device_train_batch_size=1,
        learning_rate=5e-5,
        logging_steps=10,
        save_steps=100,
        num_generations=8,  # How many groups for each prompt
        max_new_tokens=128,  # Stops generating after 128 output tokens
    )
    ...
    trainer = MAGRPOTrainer(
        ...
        reward_processors=RewardProcessors.scale(
            factor=100.0
        ),  # Apply transformation to raw rewards
        args=config,
